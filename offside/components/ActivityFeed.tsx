  ...
  return (
    <div className="bg-card border border-border rounded-xl">
      ...
      <div className="divide-y divide-border max-h-72 overflow-y-auto">
        ...
        {data?.length === 0 && <p className="text-xs text-steel text-center py-6">No predictions yet — be the first.</p>}
        {data?.map((item: any, i: number) => (
          <div key={i} className="px-4 py-2.5 text-xs">
            <span className="text-warm font-medium">{item.username}</span>
            <span className="text-steel"> predicted </span>
            <span className="text-warm">{item.match}</span>
            {item.boosted && <span className="text-indigo-mid"> · boosted</span>}
